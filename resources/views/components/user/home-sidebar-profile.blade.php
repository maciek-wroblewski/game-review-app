<div class="card premium-dashboard-card border-0 mb-4 overflow-hidden">
    <div class="p-4 bg-dark text-white position-relative" style="background: linear-gradient(135deg, #0d6efd 0%, #083c8c 100%);">
        <div class="d-flex align-items-center gap-3">
            <x-user.static-avatar :user="$user" size="65px" class="border border-2 border-white shadow" />
            <div>
                <h5 class="fw-bold mb-0 text-white">{{ $user->username }}</h5>
                @if($user->is_admin)
    ...
    <div class="card-body p-4 bg-white">
        ...
        <div class="row text-center g-2 mb-4">
            <div class="col-4">
                <div class="fw-bold fs-5 text-dark">
                    {{ $user->posts_count + $user->reviews_count }}
                </div>
        ...
        <div class="d-grid gap-2">
            <a href="/users/{{ $user->username }}" class="btn btn-sm btn-outline-primary rounded-pill">
