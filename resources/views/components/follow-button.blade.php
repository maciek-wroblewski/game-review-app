...
<form action="/users/{{ $targetUser->id }}/follow"
      method="POST"
      class="m-0 ajax-follow-form d-inline-block"
    ...
    <button type="submit"
        ...
        <span class="follow-text d-inline-block">
    ...
    document.addEventListener('submit', function (e) {
        ...
        if (followForm) {
            ...
            const matchingForms = document.querySelectorAll(
                `.ajax-follow-form[data-user-id="${targetUserId}"]`
            );

            // 2. Lock and trigger visual transition preparations
            ...
            .then(data => {
                ...
                // Sync follower counters
                const followerCounters = document.querySelectorAll(
                    `.followers-count[data-user-id="${targetUserId}"]`
                );

                followerCounters.forEach(counter => {

                    let currentCount = parseInt(
                        counter.textContent.replace(/,/g, '') || 0,
                        10
                    );
