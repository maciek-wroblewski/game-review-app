export default class PostFormActions {
                                    ...
                                    if (parentId) {
                                        ...
                                        if (parentCard) {
                                            ...
                                            if (repliesContent) {
                                                ...
                                                // Ensure the toggle button is active
    ...
    handleSubmit(e) {
        ...
        // 1. Check for post deletion
        if (action.match(/\/posts\/\d+/)) {
            const methodInput = form.querySelector('input[name="_method"]');
            const method = methodInput ? methodInput.value : form.getAttribute('method');

            if (method && method.toUpperCase() === 'DELETE') {
                e.preventDefault();
                this.handleDelete(form, action);
            }
        }

        // 2. Check for admin action
        if (action.match(/\/admin\/posts\/\d+\/(pin|lock)/)) {
            e.preventDefault();
            this.handleAdminAction(form, action);
        }
    }

    async handleDelete(form, action) {
        const csrfToken = form.querySelector('input[name="_token"]')?.value || this.csrfToken;
    ...
    async handleAdminAction(form, action) {
        const csrfToken = form.querySelector('input[name="_token"]')?.value || this.csrfToken;
        const postIdMatch = action.match(/\/admin\/posts\/(\d+)\//);
        if (!postIdMatch) return;
        const postId = postIdMatch[1];
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.html) {
                    ...
                    const newCard = doc.body.firstElementChild;
                    if (newCard) {
                        const wrapper = document.querySelector(`.js-post-card[data-post-id="${postId}"]`);
                        if (wrapper) {
            ...
            console.error(err);
            alert('Error performing admin action: ' + err.message);
